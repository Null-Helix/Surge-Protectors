...
clustered_df %>% group_by(jump_cluster) %>%
  mutate(max_capacity = max(capacity),
    min_capacity = min(capacity),
    change_in_capacity = max(capacity) - min(capacity),
    discharge_rate = change_in_capacity/mins)
```
...
spo2_cluster_lengths <- clustered_df %>%
  filter(hostName %in% c("44-4b-5d-01-04-29", "44-4b-5d-01-04-91",
               ...
               "44-4b-5d-01-04-39"), device == "SPO2SENSOR", jump_cluster == 1) %>%
...
```{r, echo = FALSE}
hist(spo2_cluster_lengths$discharge_rate, xlab = "Capacity per Min", main = "Distribution of Discharging Rate for SPO2SENSOR Batteries")
```

```{r, echo=FALSE}
boxplot(spo2_cluster_lengths$discharge_rate, main="Distribution of Capacity per Min SPO2SENSOR Batteries", ylab="Capacity per Minute")
```
...
```{r}
# Generate timestamps
timestamps <- seq(as.POSIXct("2024-04-01 00:00:00"), as.POSIXct("2024-04-02 00:00:00"), by = "15 min")

# Example data frame
df <- data.frame(
  Timestamp = timestamps,
  Voltage = runif(length(timestamps), 3.5, 4.2),
  Current = runif(length(timestamps), 0, 1),
  Temperature = runif(length(timestamps), 25, 35),
  Capacity = c(100:1) # Adjusted the length to match timestamps
)

# Threshold for capacity to identify discharging periods
capacity_threshold <- 70

# Create a new column to identify discharging time periods
df$Discharging_Period <- ifelse(df$Capacity <= capacity_threshold, 1, 0)

# Identify groups of discharging periods
df$Discharging_Group <- cumsum(c(0, diff(df$Discharging_Period) != 0))
