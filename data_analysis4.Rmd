...
$H_o:$ The distribution of Discharge Rates of SPO2 Device Batteries is the same as the distribution of Discharge Rate of Resp Device Batteries

$H_a:$ The distribution of Discharge Rates of SPO2 Device Batteries is greater than the distribution of Discharge Rate of Resp Device Batteries
...
```{r}
require(car)
qqPlot(resp_filtered_df$discharge_rate, ylab = "Discharge Rates for RESP device batteries")
```

```{r}
require(car)
qqPlot(spo2_filtered_df$discharge_rate, ylab = "Discharge Rates for SPO2 device batteries")
```
...
hist(spo2_filtered_df$discharge_rate, main = "Discharge Rates for SPO2 Sensor Batteries", xlab = "Discharge Rate (Capacity/Minute)", ylab = "Frequency", xlim = c(0, 0.1))

hist(resp_filtered_df$discharge_rate, main = "Discharge Rates for RESP Sensor Batteries", xlab = "Discharge Rate (Capacity in Percentage/Minute)", ylab = "Frequency", xlim = c(0, 0.1))
...
```{r}
(s_s = sd(spo2_filtered_df$discharge_rate))
```

```{r}
(s_r = sd(resp_filtered_df$discharge_rate))
```
...
$\frac{\sigma_s}{\sigma_r} = \frac{0.01164306}{0.001791995} = 6.49726$

```{r}
wilcox.test(spo2_filtered_df$discharge_rate, resp_filtered_df$discharge_rate, mu = 0, alternative = "greater")
```
